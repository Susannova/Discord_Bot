...
play_HelpText_long = (
    "und benachrichtigt alle User, die dieses Spiel auf dem Server aboniert haben. Auf die Anfrage kann mit "
    + "_join_"
    + " und "
    + "_pass_"
    + " reagiert werden, wodurch der Ersteller und bereits beigetretene Spieler eine Benachrichtigung erhalten."
)

play_HelpText = HelpText(
    "Erstellt eine Spielanfrage "
    + play_HelpText_long
    + " Der Ersteller und die Spieler bekommen kurz vor Beginn des Spieles eine Nachricht.",
    "Erstellt eine Spielanfrage",
    "game ('now' | time)",
)

create_clash_HelpText = HelpText(
    "Erstellt eine Clash-Anfrage für das gegebene Datum " + play_HelpText_long, "Erstellt eine Clash-Anfrage", "Datum"
)


player_HelpText = HelpText(
    "Gibt den Rang und die Winrate eines LoL-Spielers in EUW aus.", "Gibt Infos über LoL-Account aus.", "summonername"
)
...
bans_HelpText = HelpText(
    "Bestimmt die besten Bans für ein 5er LoL-Team.", "Bestimmt Bans für LoL-Team.", "name1 name2 name3 name4 name5"
)
